...
/**
 * Layout condiviso per la pagina di login.
 * Riutilizza lo stesso layout delle pagine pubbliche con SideNav laterale.
 * Struttura: barra laterale a sinistra + form di login a destra.
 * @param children - Il componente figlio (pagina di login) renderizzato nella sezione contenuto
 */
export default function Layout({ children }: { children: React.ReactNode }) {
  return (
    ...
    <div className="flex h-screen flex-col md:flex-row md:overflow-hidden">
      ...
      {/* Colonna di destra: il contenuto della pagina di login */}
      {/* Occupa tutto lo spazio rimanente (flex-grow), con padding e scroll verticale su desktop */}
      <div className="flex-grow p-6 md:overflow-y-auto md:p-12">
        {/* Renderizza il contenuto della pagina di login */}
        {children}
      </div>
    </div>
  );
