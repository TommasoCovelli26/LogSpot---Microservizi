...
async function connectToDatabase() {
  ...
  // Valida la configurazione solo quando serve davvero aprire una connessione.
  if (!mongoUri) {
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose.connect(mongoUri, opts).then((mongoose) => { // Connetti a MongoDB usando Mongoose e le opzioni specificate
      return mongoose;
    });
  }
